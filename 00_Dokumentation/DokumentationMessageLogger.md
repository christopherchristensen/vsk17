...
Der `LoggerServer` wird vorallem mit dem `DemoLogger` getestet. Dieser schickt vier LogMeldungen mit unterschiedlichen `LogLevels` an den Server. Manuell wird dann überprüft, ob die richten Meldungen erhalten wurden. Dieser Test dient hauptsächlich zur Überprüfung der TCP Verbindung und dem LogMessage-Handling in der Queue
